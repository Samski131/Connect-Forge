@tool
extends Node
@export_tool_button("Rebuild Board") var rebuild_board_action = rebuild_board
...
func build_board():
	Global.board_settings.board_height= rows
	Global.board_settings.board_width = collumns
	for x in range(collumns):
		for y in range(rows):
			create_slot(x,y)
			Global.boardPool.board.append(null)
...
func setup_board_settings():
	...
	Global.board_settings.board_collumns = collumns
	Global.board_settings.board_rows = rows
func create_slot(x:int, y:int):
	...
	board_pool.call_deferred("add_child", new_slot)
