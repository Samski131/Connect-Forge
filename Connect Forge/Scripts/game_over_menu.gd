...
const DEFAULT_PLAYER_SCORE_ROW_SCENE:PackedScene = preload(
	"res://Scenes/User Interface/player_score_row.tscn"
)
...
var popup_delay_seconds:float = 1.5

@export_group("Palette Indices")
@export var frame_colour_index:int = 3
@export var frame_border_colour_index:int = 1
@export var winner_label_colour_index:int = 3
...
# Incremented whenever a new delayed opening starts.
# This prevents an old timer from reopening the menu later.
...
func _ready() -> void:
	...
	if next_round_button != null and next_round_button.pressed.is_connected(_on_next_round_pressed) == false:
...
func show_game_over(new_winner_id:int) -> void:
	...
	# Prepare everything while the popup remains hidden.
	setup_winner_token()
	setup_winner_name(winner_palette)
	setup_outer_frame(winner_palette)
	rebuild_score_rows()

	# Keep both the backdrop and popup hidden during the board-viewing delay.
	visible = false

	if popup_delay_seconds > 0.0:
		await get_tree().create_timer(popup_delay_seconds).timeout

	# Do not show it if this request was cancelled or superseded.
...
func setup_winner_name(winner_palette:ColorPalette) -> void:
	...
	var winner_colour:Color = get_palette_colour(
		winner_palette,
		winner_label_colour_index,
		Color.WHITE
	)

	# WinnerNameLabel has LabelSettings with a built-in font colour.
	# Recolouring the LabelSettings is therefore required.
...
func setup_outer_frame(winner_palette:ColorPalette) -> void:
	...
	var existing_style:StyleBoxFlat = \
		outer_frame.get_theme_stylebox("panel") as StyleBoxFlat
	...
	frame_style.bg_color = get_palette_colour(
		winner_palette,
		frame_colour_index,
		Color.WHITE
	)

	frame_style.border_color = get_palette_colour(
		winner_palette,
		frame_border_colour_index,
		Color.WHITE
	)
...
func rebuild_score_rows() -> void:
	...
	for row_index in range(row_count):
		...
		var row_position:PlayerScoreRow.RowPosition = get_row_position(
			row_index,
			row_count
		)
...
func create_score_row(
	player_id:int,
	row_position:PlayerScoreRow.RowPosition
) -> void:
	...
	if new_row == null:
		...
		push_error(
			"GameOverMenu: player_score_row.tscn root must use PlayerScoreRow."
		)
		return
...
func get_player_name(player_id:int) -> String:
	...
	if player_id >= 0 and player_id < game_manager.player_names.size():
		var player_name:String = str(
			game_manager.player_names[player_id]
		).strip_edges()
...
func get_player_score(player_id:int) -> int:
	...
	if player_id >= 0 and player_id < game_manager.player_wins.size():
		return int(game_manager.player_wins[player_id])
...
func get_player_palette(player_id:int) -> ColorPalette:
	...
	return game_manager.player_colours[player_id]


func get_palette_colour(
...
func is_valid_player_id(player_id:int) -> bool:
	...
	if player_id >= game_manager.number_of_players:
		return false
