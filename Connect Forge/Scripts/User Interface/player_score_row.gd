...
const LIGHT_ROW_STYLE:StyleBox = preload(
	"res://Assets/User Interface/Resources/Theme Presets/Game Over Menu/Light_Player_Row_Panel.stylebox"
)

const DARK_ROW_STYLE:StyleBox = preload(
	"res://Assets/User Interface/Resources/Theme Presets/Game Over Menu/Dark_Player_Row_Panel.stylebox"
)

const STRIP_STYLE:StyleBox = preload(
	"res://Assets/User Interface/Resources/Theme Presets/Game Over Menu/colour chip on score panel.stylebox"
)
...
func _ready() -> void:
	# Prevent children drawing beyond the rounded outer row.
	clip_contents = true
...
func setup_text() -> void:
	...
	if winner_star_label != null:
		...
		if stored_is_winner:
			...
			var star_settings:LabelSettings = winner_star_label.label_settings.duplicate(true) as LabelSettings
...
func setup_row_style() -> void:
	...
	if row_style == null:
		push_error(
			"PlayerScoreRow: Player row style must be a StyleBoxFlat."
		)
		return
